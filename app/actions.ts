...
// --- 1. CREATE ROOM (Football Only for now) ---
export async function createRoom(hostName: string) {
  const supabase = await createClient()
  const safeHostName = hostName || 'Host'

  // SAFEGUARD: Added .eq('sport', 'football') to prevent NBA players from appearing
...
export async function advanceRound(roomCode: string) {
  ...
  // SAFEGUARD: Added .eq('sport', 'football')
  const { count } = await supabase
    .from('players')
    .select('*', { count: 'exact', head: true })
    .gt('rating', 0)
