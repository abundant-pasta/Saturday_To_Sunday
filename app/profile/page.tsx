...
export default function ProfilePage() {
  ...
  const [stats, setStats] = useState({
    ...
    bestScore: 0,
    currentStreak: 0,
    maxStreak: 0 // You might need to add this column to DB, or we just track current
  })
  ...
  useEffect(() => {
    const getUserAndProfile = async () => {
      ...
      const { data: profile } = await supabase
        .from('profiles')
        .select('username, show_avatar, current_streak, max_streak')
      ...
      if (profile) {
        ...
        setStats({
            ...
            maxStreak: profile.max_streak || (profile.current_streak || 0) // Fallback if column missing
        })
      }
      ...
      <div className="max-w-md mx-auto flex flex-col gap-6">
        ...
        <div className="grid grid-cols-2 gap-3">
            ...
            <div className="bg-neutral-900/50 border border-neutral-800 rounded-xl p-4 flex flex-col items-center justify-center gap-1 hover:border-blue-500/30 transition-colors">
             ...
             {/* CURRENT STREAK */}
             <div className="bg-neutral-900/50 border border-neutral-800 rounded-xl p-4 flex flex-col items-center justify-center gap-1 hover:border-orange-500/30 transition-colors">
                <div className="flex items-center gap-2 text-orange-500 mb-1">
                    <Flame className="w-4 h-4" />
                    <span className="text-[10px] font-bold uppercase tracking-widest">Streak</span>
                </div>
                <span className="text-3xl font-black text-white font-mono">{stats.currentStreak}</span>
            </div>

             {/* MAX STREAK (Assuming logic exists) */}
             <div className="bg-neutral-900/50 border border-neutral-800 rounded-xl p-4 flex flex-col items-center justify-center gap-1 hover:border-purple-500/30 transition-colors">
                <div className="flex items-center gap-2 text-purple-400 mb-1">
                    <Zap className="w-4 h-4" />
                    <span className="text-[10px] font-bold uppercase tracking-widest">Max Streak</span>
                </div>
                <span className="text-3xl font-black text-white font-mono">{stats.maxStreak}</span>
            </div>
        </div>
