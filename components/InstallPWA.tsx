...
import { X, Share, PlusSquare, MoreHorizontal, Download, Smartphone } from 'lucide-react'
...
export default function InstallPwa({ mode = 'button' }: InstallPwaProps) {
    ...
    useEffect(() => {
        ...
        // 4. Check for Banner Dismissal
        if (localStorage.getItem('s2s_install_banner_dismissed')) {
            setIsDismissed(true)
        }
    ...
    const handleDismiss = () => {
        setIsDismissed(true)
        localStorage.setItem('s2s_install_banner_dismissed', 'true')
    }

    // If installed, show nothing
    if (isStandalone) return null

    // If banner mode and dismissed, show nothing
    if (mode === 'banner' && isDismissed) return null

    // --- MODE: BANNER (Sticky Bottom) ---
    if (mode === 'banner') {
        return (
            <>
                <div className="fixed bottom-4 left-4 right-4 z-[49] md:hidden animate-in slide-in-from-bottom-full duration-500">
                    <div className="bg-neutral-900/95 backdrop-blur-md border border-[#00ff80]/30 rounded-2xl p-4 shadow-2xl flex items-center gap-4 relative ring-1 ring-[#00ff80]/10">

                        {/* Close Button */}
                        <button
                            onClick={handleDismiss}
                            className="absolute -top-2 -right-2 bg-neutral-800 text-neutral-400 rounded-full p-1 border border-neutral-700 hover:text-white"
                        >
                            <X className="w-4 h-4" />
                        </button>

                        <div className="w-10 h-10 bg-[#00ff80]/10 rounded-xl flex items-center justify-center shrink-0 border border-[#00ff80]/20">
                            <Download className="w-5 h-5 text-[#00ff80]" />
                        </div>

                        <div className="flex-1">
                            <h3 className="text-white font-black uppercase text-xs tracking-wider">Don't break your streak!</h3>
                            <p className="text-neutral-400 text-[10px] leading-tight mt-0.5">
                                Install the app to get daily reminders.
                            </p>
                        </div>
                        ...
                        <Button
                            ...
                            size="sm"
                            className="bg-[#00ff80] text-black font-black uppercase tracking-wider text-[10px] h-8 px-3 hover:bg-[#05ff84]"
                        >
                            Install
                        </Button>
                    </div>
                ...
                <InstallModal isOpen={isOpen} setIsOpen={setIsOpen} isIOS={isIOS} isChromeIOS={isChromeIOS} />
            </>
        )
    }
...
function InstallModal({ isOpen, setIsOpen, isIOS, isChromeIOS }: any) {
            ...
            <div className="bg-neutral-900 border border-neutral-800 rounded-2xl w-full max-w-sm p-6 relative shadow-2xl animate-in zoom-in-95 duration-200 ring-1 ring-white/10">
                ...
                <div className="flex flex-col items-center text-center mb-6">
                    ...
                    <p className="text-sm text-neutral-400 mt-2 leading-relaxed">
                        Add <span className="text-white font-bold">Saturday to Sunday</span> to your home screen for daily reminders.
                    </p>
                </div>
